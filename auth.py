...
def delete_project(project_id):
    ...
    try:
        ...
        # Get project details and user_id for folder deletion
        c.execute("SELECT user_id FROM projects WHERE id = ?", (project_id,))
        user_id = c.fetchone()[0]
        
        # Get all generations to delete files
        c.execute("SELECT id, image_path, model_path FROM generations WHERE project_id = ?", (project_id,))
        generations = c.fetchall()
        ...
        c.execute("DELETE FROM generations WHERE project_id = ?", (project_id,))
        ...
        project_folder = f"output/users/{user_id}/{project_id}"
